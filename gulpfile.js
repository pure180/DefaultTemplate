(function() {
  'use-strict';
  var autoprefixer, bower, cached, changed, cleanCSS, coffeTask, coffee, concat, connect, copy, data, del, extend, filter, fs, ftp, getDateTime, glob, gulp, gulpif, gutil, header, imagemin, jadeTask, javaScriptTask, jshint, less, lessTask, mainBowerFiles, notifier, notify, path, pkg, plumber, pug, puginheritance, rename, replace, root, runSequence, settings, sourcemaps, through, timestamp, uglify, uploadFiles;
  ...
  gulp.task('reload', ['build', 'bower:task']);
  ...
  gulp.task('bower:css', function() {
    return gulp.src(mainBowerFiles('**/*.css')).pipe(gulp.dest('./temp')).pipe(notify({
      message: notifier.served
    }));
  });

  gulp.task('bower:less:temp', ['bower:css'], function(cb) {
    var bowerLess, name;
    name = '';
    bowerLess = gulp.src(mainBowerFiles('**/*.less')).pipe(plumber(function(error) {
      gutil.log(error.message);
      this.emit('end');
    })).pipe(through.obj(function(file, enc, cb) {
      var dir, pathToVariables, variables;
  ...
  gulp.task('bower:css:temp', function() {
    return gulp.src('./temp/*.css').pipe(plumber(function(error) {
      gutil.log(error.message);
      this.emit('end');
    })).pipe(autoprefixer(settings.autoprefixer)).pipe(concat('libs.css')).pipe(gulp.dest(settings.path.dest.css)).pipe(notify({
      message: notifier.served
    })).pipe(rename({
      suffix: '.min'
    })).pipe(cleanCSS({
      compatibility: 'ie8'
    })).pipe(gulp.dest(settings.path.dest.css)).pipe(notify({
      message: notifier.served
    })).pipe(connect.reload());
  });

  gulp.task('bower:less', ['bower:less:temp'], function() {
    return gulp.start('bower:css:temp');
  });
